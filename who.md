...
SSH был создан в 1995 году финским исследователем Тату Юлоненом (Tatu Ylonen). Его целью было обеспечить надёжный и защищённый способ подключения к удалённому терминалу. Со временем SSH стал стандартом де-факто для безопасного управления Unix-подобными системами, а также часто используется и в Windows-средах (через сторонние программы или встроенный клиент, начиная с Windows 10).
...
\# пример фрагмента /etc/pam.d/sshd
...
1. **Парольная аутентификация (PasswordAuthentication)**. Клиент вводит пароль, который отправляется на сервер в зашифрованном виде. Сервер сверяется с локальной базой данных, LDAP или иным механизмом через PAM.
1. **Аутентификация по ключу (PubkeyAuthentication)**. Более безопасный метод, когда у пользователя есть пара ключей (приватный хранится у него, а публичный известен серверу). Считается предпочтительным вариантом, особенно в корпоративных сетях и при работе с облачными провайдерами.
1. **Клавиатурно-интерактивная аутентификация (Keyboard-Interactive)**. Гибкая схема, позволяющая серверу задавать клиенту произвольные вопросы (например, «Введите пароль:», «Введите код подтверждения:»). На практике реализуется через PAM и другие плагины.
1. **GSS API**. Используется вместе с Kerberos в некоторых корпоративных средах. Позволяет осуществлять единый вход (Single Sign-On).
